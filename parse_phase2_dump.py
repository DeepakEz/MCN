"""Parse redis-cli XRANGE text dump -> categorized_runs_phase2.jsonl

Phase 2: CTS router + heterogeneous temperatures (T0=0.1, T1=0.5, T2=0.9)
Dump:     docker exec mcn-redis-1 redis-cli XRANGE mcn:runs - + > phase2_raw_dump.txt
"""
import json
import re
...
# Phase 2 tribe temperatures — used to enrich output records
TRIBE_TEMPS = {0: 0.1, 1: 0.5, 2: 0.9}

STREAM_ID_RE = re.compile(r"^\d+-\d+$")

dump_path = Path("phase2_raw_dump.txt")
if not dump_path.exists():
    print(f"[error] {dump_path} not found.")
    print("Run: docker exec mcn-redis-1 redis-cli XRANGE mcn:runs - + > phase2_raw_dump.txt")
    raise SystemExit(1)
...
# Enrich with category, temperature, passed flag
for rec in records:
...
# Write JSONL
out = Path("categorized_runs_phase2.jsonl")
...
print(f"\nPer-tribe routing (Phase 2 temps: T0=0.1, T1=0.5, T2=0.9):")
for t in sorted(by_tribe):
    ...
    print(f"  T{t} (temp={temp}): {s['total']:4d} tasks ({pct_route:5.1f}% routed) | "
...
for w in range(4):
    ...
    if total_w == 0:
        break
    parts = " | ".join(f"T{t}(T={TRIBE_TEMPS.get(t,'?')}):{100*tc[t]/total_w:.0f}%" for t in sorted(tc))
